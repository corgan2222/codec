...
function milesight(bytes) {
    ...
    for (var i = 0; i < bytes.length; ) {
        ...
        // LIGHT
            ...
            decoded.visible_and_infrared = readUInt16LE(bytes.slice(i + 2, i + 4));
        ...
        // SENSOR ENABLED
            ...
            var enabled = bytes[i];
            var mask = bytes[i + 1];
            decoded.temperature_enable = (mask >> 0) & 0x01;
            decoded.humidity_enable = (mask >> 1) & 0x01;
            decoded.pir_enable = (mask >> 2) & 0x01;
            decoded.illumination_enable = (mask >> 3) & 0x01;
            decoded.co2_enable = (mask >> 4) & 0x01;
            decoded.tvoc_enable = (mask >> 5) & 0x01;
            decoded.pressure_enable = (mask >> 6) & 0x01;
            i += 2;
        } else {
            break;
        }
